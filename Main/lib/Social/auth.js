...
exports.index = async (req, res, next) => {
    var { username, password, twofactor = '0', _2fa, type = 'both' } = req.query;
    ...
    if (!username || !password) return res.json({
        ...
        usage: {
            ...
            parameters: {
                username: "Email Facebook",
                password: "Mật khẩu Facebook", 
                twofactor: "Mã bí mật 2FA (tùy chọn)",
                _2fa: "Mã 2FA hiện tại (tùy chọn)",
                type: "token|appstate|both (mặc định: both)"
            },
            examples: [
                "/facebook/auth?username=example@gmail.com&password=yourpass123",
                "/facebook/auth?username=example@gmail.com&password=yourpass123&_2fa=123456",
                "/facebook/auth?username=example@gmail.com&password=yourpass123&type=token",
                "/facebook/auth?username=example@gmail.com&password=yourpass123&type=appstate"
            ]
        }
    });
    ...
    try {
        ...
        return new Promise((resolve) => {
            axios.request(options).then(async(response) => {
                try {
                    // Xử lý data response
                    const processedData = await processResponse(response.data, type);
                    return res.json({
                        status: true,
                ...
                if(error.response?.data?.error?.code == 401) {
                    return res.json({
                        ...
                        error_code: 401,
                        debug: 'Unauthorized - Check username/password'
                    });
                }
                
                // Kiểm tra nếu tài khoản bị checkpoint
                if(error.response?.data?.error?.message?.includes('checkpoint')) {
                    return res.json({
                        status: false,
                        message: 'Tài khoản Facebook bị checkpoint. Vui lòng đăng nhập Facebook trên trình duyệt để xác minh.',
                ...
                if(twofactor == '0' && (!_2fa || _2fa == "0")) {
                    return res.json({
                        status: false,
                        message: 'Tài khoản yêu cầu xác thực 2 lớp. Vui lòng thêm tham số twofactor hoặc _2fa!',
                        usage_2fa: {
                            example_with_secret: "/facebook/auth?username=email@gmail.com&password=pass&twofactor=YOUR_2FA_SECRET",
                            example_with_code: "/facebook/auth?username=email@gmail.com&password=pass&_2fa=123456"
                        }
                    });
                }
                ...
                axios.request(options).then(async(response) => {
                    ...
                    console.log('2FA Login error:', error.response?.data);
                    return res.json({
                        status: false,
                        message: 'Lỗi xác thực 2FA. Kiểm tra lại mã 2FA.',
                        error_details: error.response?.data,
                        suggestions: [
                            'Kiểm tra mã 2FA có đúng không',
                            'Đảm bảo twofactor secret chính xác',
                            'Thử đăng nhập Facebook trên trình duyệt trước'
                        ]
                    });
                })
            })
        ...
        return res.json({
            status: false,
            message: 'Lỗi hệ thống. Vui lòng kiểm tra lại thông tin đăng nhập!',
            error_type: 'system_error',
            suggestions: [
                'Kiểm tra username (email) và password',
                'Đảm bảo tài khoản Facebook không bị khóa',
                'Thử đăng nhập Facebook trên trình duyệt trước',
                'Nếu có 2FA, thêm tham số twofactor hoặc _2fa'
            ]
        });
    }
}

async function processResponse(data, type) {
    const result = {};
    
    if (type === 'token' || type === 'both') {
        result.access_token = data.access_token;
        result.access_token_eaad6v7 = await convertToken(data.access_token);
        result.uid = data.uid;
        result.machine_id = data.machine_id;
        result.cookies = await convertCookie(data.session_cookies);
    }
    
    if (type === 'appstate' || type === 'both') {
        result.appstate = data.session_cookies.map((e) => {
            return {
                key: e.name,
                value: e.value,
                domain: "facebook.com",
    ...
    if (type === 'both') {
        result.session_cookies = result.appstate;
    }
