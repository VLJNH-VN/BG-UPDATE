    ...
    if (!url) return res.json({ 
        ...
        parameters: {
            required: {
                url: {
                    type: "string",
                    description: "URL video Facebook cần tải xuống",
                    example: "https://www.facebook.com/watch/?v=123456789"
                }
            }
        },
        usage: {
            endpoint: "/facebook/video",
            method: "GET",
            example: "/facebook/video?url=https://www.facebook.com/watch/?v=123456789"
        }
    });
    if (!url.includes("facebook.com")) {
      return res.jsonp({
        error: "URL không hợp lệ",
        usage: "GET /facebook/video?url=<facebook_video_url>",
        note: "Chỉ hỗ trợ video từ Facebook"
      });
    }
    axios.get(url, {
      headers: data
